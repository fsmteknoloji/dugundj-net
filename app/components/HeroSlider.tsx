...
export default function HeroSlider() {
  ...
  return (
    <div style={{ position: 'relative', height: '100vh', minHeight: '600px', overflow: 'hidden', background: '#0C0C0C' }}>
      ...
      <div style={{ position: 'absolute', bottom: 0, left: 0, width: '4px', height: '100%', background: 'linear-gradient(to top, #6B1FFF, transparent 60%)', zIndex: 6 }} />

      <div style={{
        position: 'absolute', inset: 0, zIndex: 5,
        display: 'flex', flexDirection: 'column', justifyContent: 'center',
        padding: isMobile ? '100px 24px 80px' : '120px 64px 100px',
        maxWidth: isMobile ? '100%' : '750px',
      }}>
        <div style={{ fontSize: '10px', letterSpacing: '3.5px', textTransform: 'uppercase', color: '#8B45FF', marginBottom: '20px', fontWeight: 500, display: 'flex', alignItems: 'center', gap: '12px' }}>
        ...
        <h1 style={{
          ...
          fontSize: isMobile ? 'clamp(42px,12vw,64px)' : 'clamp(54px,7.5vw,96px)',
        ...
        <p style={{ fontSize: isMobile ? '14px' : '15px', fontWeight: 300, lineHeight: 1.75, color: 'rgba(255,255,255,.62)', maxWidth: '400px', marginBottom: '44px' }}>
          {slide.desc}
        </p>

        <div style={{ display: 'flex', gap: '14px', alignItems: 'center', flexWrap: 'wrap' }}>
          <Link href={slide.btn1.href} style={{ fontSize: '11px', letterSpacing: '2px', textTransform: 'uppercase', fontWeight: 500, background: '#6B1FFF', color: '#fff', padding: '14px 34px', textDecoration: 'none', borderRadius: '2px' }}>{slide.btn1.text}</Link>
          <Link href={slide.btn2.href} style={{ fontSize: '11px', letterSpacing: '2px', textTransform: 'uppercase', color: 'rgba(255,255,255,.6)', textDecoration: 'none', borderBottom: '1px solid rgba(255,255,255,.25)', paddingBottom: '2px' }}>{slide.btn2.text}</Link>
        </div>
      </div>
      ...
      <div style={{ position: 'absolute', bottom: '36px', right: isMobile ? '20px' : '64px', zIndex: 20, display: 'flex', alignItems: 'center', gap: '28px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          {slides.map((_, i) => (
            <button key={i} onClick={() => go(i)} style={{
              ...
              backgroundClip: 'content-box', transition: 'all .35s',
            }} />
          ))}
        </div>
      </div>
      ...
      {!isMobile && (
        <>
          <button onClick={() => go(cur - 1)} style={{ position: 'absolute', left: '28px', top: '50%', transform: 'translateY(-50%)', zIndex: 20, width: '48px', height: '48px', border: '1px solid rgba(255,255,255,.15)', background: 'rgba(255,255,255,.04)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="1.5"><polyline points="15 18 9 12 15 6"/></svg>
          </button>
          <button onClick={() => go(cur + 1)} style={{ position: 'absolute', right: '28px', top: '50%', transform: 'translateY(-50%)', zIndex: 20, width: '48px', height: '48px', border: '1px solid rgba(255,255,255,.15)', background: 'rgba(255,255,255,.04)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
